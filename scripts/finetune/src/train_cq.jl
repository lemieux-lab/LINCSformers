...
function train(model, opt, data, config, logs)
    ...
    start_epoch = hasproperty(config, :start_epoch) ? config.start_epoch : 1

    mkpath(joinpath(save_dir, "checkpts"))

    # for epoch in ProgressBar(1:epochs)
    for epoch in ProgressBar(start_epoch:epochs)
        println("epoch $epoch")
        ...
        if is_checkpt
            ...
            path = joinpath(save_dir, "checkpts", "$(pt)_epoch_$(epoch).jld2")
            jldsave(path; 
